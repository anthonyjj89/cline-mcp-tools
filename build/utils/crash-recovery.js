 ...
 */
async function attemptRecovery(filePath) {
    try {
        // Strategy 1: Try direct JSON parsing (using existing functionality)
        try {
            return await readJsonArray(filePath);
        }
        catch (error) {
        ...
        try {
            ...
            console.warn(`Chunk-by-chunk parsing failed: ${error.message}`);
        }
        ...
        try {
            ...
            console.warn(`Line-by-line parsing failed: ${error.message}`);
        }
        ...
        console.error(`All recovery strategies failed: ${error.message}`);
        return [];
    }
}
/**
 ...
 * @returns Promise resolving to the recovered messages
 */
async function recoverByChunks(filePath) {
 ...
 * @returns Promise resolving to the recovered messages
 */
async function recoverByLines(filePath) {
    return new Promise((resolve, reject) => {
        try {
 ...
 * @returns Promise resolving to the recovered messages
 */
async function extractValidJsonObjects(filePath) {
    return new Promise((resolve, reject) => {
        try {
            // Read the file
            const fileContent = fs.readFileSync(filePath, 'utf8');
 ...
 */
function extractOriginalTask(message) {
    let content = '';
 ...
 */
function extractCodeEvolution(messages) {
 ...
 */
function generateEvolutionDescription(snippets) {
    if (snippets.length <= 1) {
        return '';
    }
    const firstSnippet = snippets[0];
 ...
 */
function analyzeTopics(topics, messages) {
    if (topics.length === 0) {
 ...
 */
function identifyActiveFiles(messages, allFiles) {
                ...
                if (sentence.includes('?')) {
                    ...
                    for (let j = i + 1; j < recentMessages.length; j++) {
                        ...
                        if (responseMessage.role === 'assistant') {
                            ...
                            const matchCount = questionWords.filter(word => responseContent.toLowerCase().includes(word)).length;
 ...
 */
function extractRecentMessages(messages, count) {
 ...
 */
function determineCurrentStatus(messages) {
 ...
 */
function extractQuestionTopic(question) {
 ...
 */
function identifyDecisionPoints(messages) {
 ...
 */
function extractLatestState(messages, messageCount = 5) {
    // Get the last few messages
 ...
 */
export function formatRecoveredContext(recoveredContext) {
