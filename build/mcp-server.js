...
}).refine(data => data.task_id !== undefined || data.active_label !== undefined, {
    ...
    if (!task_id || isRequestingActiveA || isRequestingActiveB) {
        ...
        if (activeTasksData && activeTasksData.activeTasks && activeTasksData.activeTasks.length > 0) {
            ...
            if (isRequestingActiveB) {
                ...
                activeTask = activeTasksData.activeTasks.find((t) => t.label === 'B');
                if (activeTask) {
                    activeLabel = 'B';
                }
            }
                ...
                activeTask = activeTasksData.activeTasks.find((t) => t.label === 'A') ||
        ...
        try {
            ...
            if (activeTasksData && activeTasksData.activeTasks && activeTasksData.activeTasks.length > 0) {
                ...
                const activeTask = activeTasksData.activeTasks.find((t) => t.label === 'A') ||
                    activeTasksData.activeTasks.find((t) => t.label === 'B');
                if (activeTask) {
                    targetTaskId = activeTask.id;
                    isActiveTask = true;
...
async function handleGetActiveTask(args) {
    ...
    if (task_id && activeTasksData && activeTasksData.activeTasks && activeTasksData.activeTasks.length > 0) {
        const activeTask = activeTasksData.activeTasks.find((t) => t.id === task_id);
        if (activeTask) {
    ...
    if (activeTasksData && activeTasksData.activeTasks && activeTasksData.activeTasks.length > 0) {
        ...
        if (filteredTasks.length > 0) {
            ...
            const taskDetails = await Promise.all(filteredTasks.map(async (taskInfo) => {
            ...
            content: [
                {
                    type: 'text',
                    text: JSON.stringify({
                        error: `Failed to recover crashed chat: ${error.message}`,
                        task_id
                    }, null, 2),
                },
            ],
