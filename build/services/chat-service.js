...
import { getChatDirectory, getApiConversationFilePath, getUiMessagesFilePath, formatFileSize } from '../utils/paths-chat.js';
 ...
 */
export async function listChats(chatsDir) {
    try {
 ...
 */
export async function getChat(chatsDir, chatId) {
    try {
        ...
        throw new Error(`Failed to get chat ${chatId}: ${error.message}`);
    }
}
/**
 * Get the latest chat
 ...
 */
export async function getLatestChat(chatsDir) {
    try {
        ...
        throw new Error(`Failed to get latest chat: ${error.message}`);
    }
}
/**
 ...
 */
export async function getChatSummary(chatsDir, chatId) {
    try {
        // Get chat details
        ...
        const previewMessages = await getConversationHistory(chatsDir, chatId, { limit: 10 });
        ...
        const humanMessages = await getConversationHistory(chatsDir, chatId, {
            limit: Number.MAX_SAFE_INTEGER
        }, (message) => message.role === 'human');
        const assistantMessages = await getConversationHistory(chatsDir, chatId, {
            limit: Number.MAX_SAFE_INTEGER
        ...
        throw new Error(`Failed to generate chat summary: ${error.message}`);
    }
}
