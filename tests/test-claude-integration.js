...
/**
 * Test script for the Claude Task Reader MCP server integration with Claude Desktop
 ...
 */
async function updateClaudeDesktopConfig() {
  printSection('Updating Claude Desktop Configuration');
  ...
  try {
    ...
    // Create or update the claude-task-reader MCP server configuration
    if (!config.mcpServers) {
      config.mcpServers = {};
    }
    
    config.mcpServers['claude-task-reader'] = {
      command: 'node',
    ...
    printInfo('Writing updated configuration...');
    fs.writeFileSync(CONFIG_FILE_PATH, JSON.stringify(config, null, 2));
    
    printSuccess('Claude Desktop configuration updated successfully');
    printInfo('Please restart Claude Desktop to apply the changes');
    
    return true;
  } catch (error) {
    printError(`Error updating Claude Desktop configuration: ${error.message}`);
    return false;
  }
}
 ...
 */
function printTestSummary(moduleResult, startupResult, connectionResult) {
 ...
 */
async function main() {
  console.log(`${colors.bright}${colors.magenta}Claude Task Reader MCP Server Integration Test${colors.reset}\n`);
  ...
  try {
    ...
    if (!connectionResult) {
      ...
      // Ask if the user wants to update the Claude Desktop configuration
      console.log('\n');
      printInfo('Would you like to update the Claude Desktop configuration? (y/n)');
      process.stdin.once('data', async (data) => {
        ...
        if (input === 'y' || input === 'yes') {
          ...
          printInfo('Configuration update skipped');
        }
        
        printTestSummary(moduleResult, startupResult, connectionResult);
        process.exit(0);
      });
    } else {
      printTestSummary(moduleResult, startupResult, connectionResult);
