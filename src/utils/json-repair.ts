...
export function repairJson(jsonString: string): string {
  ...
  try {
    ...
    logDebug('Initial JSON parsing failed, attempting repair', { error: (initialError as Error).message });
    ...
    try {
      ...
      logWarning('Failed to parse JSON even after repair attempts', { error: (repairError as Error).message });
...
export function parseJsonWithRepair(jsonString: string): any {
  ...
  try {
    ...
    logWarning('Direct JSON parsing failed, attempting repair', { error: (error as Error).message });
    try {
      ...
      logError('Failed to repair and parse JSON', { error: (repairError as Error).message });
      return null;
    }
  }
}
